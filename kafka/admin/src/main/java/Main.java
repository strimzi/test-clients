...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        switch (config.getTopicOperation().toLowerCase()) {
            case "list":
                listTopics(admin);
                break;
            case "delete":
    ...
    static void createTopic(Admin admin, AdminConfiguration config, int topicsNumber, int topicOffset) {
        ...
        for (int i = 0; i < topicsNumber; i++) {
            ...
            // Create a compacted topic(s)
            log.info("Creating: " + topicName);
            topics.add(new NewTopic(topicName, Integer.parseInt(config.getTopicPartitions()), Short.parseShort(config.getReplicationFactor()))
